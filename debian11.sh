...
function block_unsafe_ports() {
    echo "一键屏蔽不安全的端口和服务..."
    iptables -A INPUT -p tcp --dport 23 -j DROP  # 屏蔽 Telnet
    iptables -A INPUT -p tcp --dport 2049 -j DROP  # 屏蔽 NFS
    iptables -A INPUT -p udp --dport 2049 -j DROP
    iptables -A INPUT -p tcp --dport 111 -j DROP  # 屏蔽 RPC
    iptables -A INPUT -p udp --dport 111 -j DROP
    iptables -A INPUT -p tcp --dport 69 -j DROP  # 屏蔽 TFTP
